...
public class fatec_cursoDao extends ComumDao { 
	...
	{
		try{
			PreparedStatement conn = BeginNewStatement("INSERT INTO fatec_curso (codigo, nome) values (?, ?)");
	...
	{
		try{
			PreparedStatement conn = BeginNewStatement("UPDATE fatec_curso SET codigo = ?, nome = ? WHERE codigo = " +  entidade.getCodigo());
			conn.setInt(1, entidade.getCodigo());
			conn.setString(2, entidade.getNome());
			conn.execute();
			commit();
