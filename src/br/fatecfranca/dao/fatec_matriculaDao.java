...
public class fatec_matriculaDao extends ComumDao { 
	...
	{
		try{
			PreparedStatement conn = BeginNewStatement("INSERT INTO fatec_matricula (codigoaluno, codigocurso, matricula) values (?, ?, ?)");
			conn.setInt(1, entidade.getCodigoaluno());
			conn.setInt(2, entidade.getCodigocurso());
			conn.setDate(3, (Date) entidade.getMatricula());
			conn.execute();
			commit();
		}
		catch (Exception ex){
	...
	{
		try{
			PreparedStatement conn = BeginNewStatement("UPDATE fatec_matricula SET codigoaluno = ?, codigocurso = ?, matricula = ? WHERE codigo = " +  entidade.getCodigo());
			conn.setInt(1, entidade.getCodigoaluno());
			conn.setInt(2, entidade.getCodigocurso());
			conn.setDate(3, (Date) entidade.getMatricula());
			conn.execute();
